...
class MainProcessLogger:
    ...
    def info(self, msg):
        """
        Print information only in the main process
        Args:
    ...
    def debug(self, msg):
        """
        Print debug information only in the main process.
        If it's the main process, print the message to console; otherwise do nothing.

        Args:
            msg (str): Debug message string to be printed.
    ...
    def warning(self, msg):
        """
        Print warning information in the main process and log to file.
        If not the main process, no information will be printed.
...
class BaseTrainer(object):
    ...
    def __init__(self, args=None, cfg=None):
        ...
        Args:
            args (Optional[argparse.Namespace], optional): Arguments parsed by argparse. Defaults to None.
        ...
        with self.accelerator.main_process_first():
            if args.resume:
                if args.resume_from_ckpt_path == "" or args.resume_from_ckpt_path is None:
                    pass
                else:
    ...
    def _accelerator_prepare(self):
        """
        After the accelerator is ready, pass the dataset, model, optimizer and learning rate scheduler to the accelerator.
        Returns a tuple including: training dataset dataloader, validation dataset dataloader, model, optimizer and learning rate scheduler.

        Args:
            None

        Returns:
            tuple (DataLoader, DataLoader, nn.Module, optim.Optimizer, torch.optim.lr_scheduler._LRScheduler):
                Training dataset dataloader, validation dataset dataloader, model, optimizer and learning rate scheduler.
        """
        (
            self.model,
            self.optimizer,
    ...
    def save_checkpoint(self):
        """
        Save model checkpoint, only executed in the main process.
        If the number of kept checkpoints exceeds keep_last, delete the earliest checkpoint.
        ...
        if self.accelerator.is_main_process:
            ...
            self.model_module.config.save_pretrained(path)
            self.logger.info(f"Config file saved to {path}")
    ...
    def get_lr(self, it):
        """
        Get the current iteration's learning rate according to the strategy specified in the config file.
        Args:
            it (int): Current iteration count, starting from 0.
        Returns (float): Current iteration's learning rate. If current iteration is greater than total iterations, return minimum learning rate; otherwise, calculate current iteration's learning rate according to linear warmup and cosine decay strategy.
        """
        # Check if DEBUG mode is enabled
        debug_mode = os.environ.get("DEBUG", "").lower() in ("1", "true", "yes")
        
        # In debug mode, use constant learning rate of 2e-3
        if debug_mode:
            return 2e-3
    ...
    def _train_epoch(self):
        ...
        if debug_mode:
            ...
            self.cfg.train.save_checkpoint_stride[0] = 100
            print("[DEBUG] Checkpoint settings overridden - saving every 100 steps, keeping only 1 checkpoint")
        else:
        ...
        while True:
            try:
                ...
                print(e, "End of DataLoader reached for the epoch.")
                break
            except Exception as e:
                raise e
            start_this_step_time = time.time()
            ...
            stats = {"Time/load batch": start_this_step_time - finish_last_step_time,
                     ...
                     'Time/load audio': batch.get('load_audio_time', -1)}
            ...
            # Print loss info in debug mode
            if debug_mode and self.accelerator.is_main_process:
                print(f"Step {self.step} - Current Loss: {self.current_loss:.5f}, EMA Loss: {ema_loss:.5f}, LR: {lr:.6f}")
                    ...
                    self.log(
                        {
                            ...
                            "Step/gradient_accumulation_steps": self.gradient_accumulation_steps,
                        },
                        step=self.step,
                    )
                self.step += 1
            ...
            if self.step % 200 == 0 or self.debug_mode:
                self.log(stats, step=self.step)
    ...
    def _build_dataloader(self, dataloader):
        """
        Build data loader, needs to be implemented by child class.
        This function should return an iterator containing all samples, each sample is a dictionary containing the following key-value pairs:
            - "image" (Tensor[N, C, H, W]): Input image, where N is batch size, C is number of channels, H and W are height and width.
            - "label" (Tensor[N, D]): Target label, where N is batch size, D is number of classes.
        If your model needs more information (e.g. rotation angle), please add it here.

        Raises:
            NotImplementedError: Raised when this method is not implemented by child class.

        Returns:
            Iterator[Dict[str, Tensor]]: An iterator containing all samples, each sample is a dictionary containing the following key-value pairs:
                - "image" (Tensor[N, C, H, W]): Input image, where N is batch size, C is number of channels, H and W are height and width.
                - "label" (Tensor[N, D]): Target label, where N is batch size, D is number of classes.
        """
        # setup data_loader
    ...
    def _init_accelerator(self):
        """
        Initialize accelerator, including project configuration and distributed data parallel parameters.
        If it's the main process, create log folder.

        Returns:
            None, no return value.
        """
        # 2. Get today's date as a string in YYYYMMDD format
        date_stamp = datetime.now().strftime("%Y%m%d")
        # 3. Combine the experiment name with the date stamp using an f-string
        exp_name_with_date = f"{self.args.exp_name}_{date_stamp}"
        self.exp_dir = os.path.join(
            os.path.abspath(self.cfg.log_dir),
            exp_name_with_date  # Use the new name here
        )
        ...
        self.gradient_accumulation_steps = self.cfg.train.gradient_accumulation_step
        if self.accelerator.is_main_process:
    ...
    def __count_parameters(model):
        ...
        if isinstance(model, dict):
            for key, value in model.items():
                model_param += sum(p.numel() for p in value.parameters() if p.requires_grad)
        else:
