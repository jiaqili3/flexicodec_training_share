...
class TransformerLM(torch.nn.Module):
    ...
    def __init__(
        ...
        self.llm_decoders = nn.ModuleList(
            [
                nn.Linear(llm_output_size, speech_token_size + 1)
    ...
    def forward(
        ...
        # pad speech token to make it divisible by num_heads
        while speech_token.shape[-1] % self.num_heads != 0:
            speech_token = torch.cat(
                [speech_token, torch.zeros_like(speech_token[:, None, 0])], dim=-1
            )
        for i in range(speech_token.shape[0]):
            while speech_token_len[i] % self.num_heads != 0:
                speech_token[i, speech_token_len[i]] = self.speech_token_size
                speech_token_len[i] += 1
        ...
        lm_targets = []
        for i in range(len(self.llm_decoders)):
            l = [
                torch.tensor(
                    [IGNORE_ID] * (2 + text_token_len[j])
                    + speech_token[j, i : speech_token_len[j] : self.num_heads].tolist()
                    + [self.speech_token_size]
                )
                for j in range(text_token.size(0))
            ]
            lm_targets.append(
                pad_sequence(l, batch_first=True, padding_value=IGNORE_ID).to(device)
            )
        ...
        speech_token = speech_token.sum(dim=2)
        ...
        mh_logits = [
            self.llm_decoders[i](lm_output) for i in range(len(self.llm_decoders))
        ]

        loss = 0

        for i in range(len(self.llm_decoders)):
            loss += self.criterion_ce(mh_logits[i], lm_targets[i])
        # acc = th_accuracy(
    ...
    def inference(
        ...
        prompt_speech_token_emb = prompt_speech_token_emb.sum(dim=2)
        prompt_speech_token_len = prompt_speech_token_len // 2
        ...
        for i in range(max_len):
            ...
            for decoder_idx in range(len(self.llm_decoders)):
                logp = self.llm_decoders[decoder_idx](y_pred[:, -1]).log_softmax(dim=-1)
                top_ids, last_sample_idx = self.sampling_ids(
                    logp.squeeze(dim=0),
                    sampling=sampling,
                    beam_size=beam_size,
                    # ignore_eos=True if (i < 30 or decoder_idx != 0) else False,
                    ignore_eos=True if (i < 30) else False,
                    # sample_idx=None if decoder_idx == 0 else last_sample_idx
                )
                ...
                if decoder_idx == 0:
                    lm_input = (
                        self.speech_embedding.weight[top_ids]
                        .clone()
                        .detach()
                        .reshape(1, 1, -1)
                    )
