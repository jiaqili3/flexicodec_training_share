...
class GeneralResidualUnit(nn.Module):
    def __init__(
        self,
        dim: int = 16,
        dilation: int = 1,
        ...
        self.block = nn.Sequential(
            ...
            SConv1d(dim, dim, kernel_size=1, norm=norm, causal=causal, pad_mode=pad_mode),
        )
...
class GeneralEncoderBlock(nn.Module):
    def __init__(
        self,
        dim: int = 16,
        stride: int = 1,
        ...
        super().__init__()
        runits = [
            GeneralResidualUnit(dim // 2, dilation=d, norm=norm, causal=causal, pad_mode=pad_mode)
            for d in dilations
        ]
        self.block = nn.Sequential(
            *runits,
...
class GeneralDecoderBlock(nn.Module):
    def __init__(
        self,
        input_dim: int = 16,
        output_dim: int = 8,
        ...
        causal=False,
        pad_mode="reflect",
        filter_type="updown",  # updown, skip, causal_updown
    ):
        super().__init__()
        self.block = nn.Sequential(
            GeneralActivation1d(
        ...
        self.block.extend(
            [
                GeneralResidualUnit(
                    output_dim, dilation=d, norm=norm, causal=causal, pad_mode=pad_mode
                )
                for d in dilations
            ]
        )
