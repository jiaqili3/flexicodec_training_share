...
class FlexiCodec(nn.Module):
    def __init__(
        ...
        # REPA loss (bottleneck path, not flow matching)
    ...
    def forward_features(self, 
        ...
        repa_loss = torch.tensor(0.0, device=audio_data.device)
        ...
        if is_aggregated:
            ...
            if self.use_bottleneck_transformer and getattr(self, "repa_mlp", None) is not None:
                repa_gt = semantic_repr_ret + acoustic_features if not bypass_quantize else semantic_repr_ret
                if repa_gt.shape[-1] != acoustic_final.shape[-1]:
                    min_t = min(repa_gt.shape[-1], acoustic_final.shape[-1])
                    repa_gt = repa_gt[..., :min_t]
                    acoustic_final_for_repa = acoustic_final[..., :min_t]
                else:
                    acoustic_final_for_repa = acoustic_final
                repa_output = self.repa_mlp(acoustic_final_for_repa.transpose(1, 2))
                repa_loss = F.mse_loss(repa_gt.transpose(1, 2), repa_output) * self.repa_loss_weight
                if self.sometimes_skip_repa and not bypass_quantize:
