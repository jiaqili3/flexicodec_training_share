...
class WNConv1d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, dilation=1, is_causal=False, **kwargs):
        super().__init__()
        self.is_causal = is_causal
        self.kernel_size = kernel_size
        self.dilation = dilation
        self.padding = (kernel_size - 1) * dilation if is_causal else (kernel_size - 1) * dilation // 2
        # Always set padding to 0 in the convolution; we handle it manually
        kwargs["padding"] = 0
        self.conv = weight_norm(nn.Conv1d(in_channels, out_channels, kernel_size, dilation=dilation, **kwargs))
    ...
    def forward(self, x):
        if self.is_causal:
            ...
            # Apply symmetric padding (centered padding)
            x = nn.functional.pad(x, (self.padding // 2, self.padding // 2))
        return self.conv(x)
...
class WNConvTranspose1d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, dilation=1, is_causal=False, **kwargs):
        ...
        self.dilation = dilation
        self.padding = (kernel_size - 1) * dilation if is_causal else (kernel_size - 1) * dilation // 2
        self.conv_transpose = weight_norm(nn.ConvTranspose1d(in_channels, out_channels, kernel_size, dilation=dilation, **kwargs))

    def forward(self, x):
        x = self.conv_transpose(x)
        if self.is_causal:
            # Remove extra padding added during transposed convolution
            crop = self.padding
            if crop > 0:
                x = x[..., :-crop]
        return x        
...
class Encoder(nn.Module):
    def __init__(
        self,
        d_model: int = 64,
        strides: list = [2,1,1,1],
        d_latent: int = 64,
        in_dim=1024,
    ):
        super().__init__()
        # Create first convolution
        self.block = [WNConv1d(in_dim, d_model, kernel_size=7, padding=3)]
