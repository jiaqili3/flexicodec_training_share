...
# DualCodec-Valle imports and setup
import sys
...
def load_dualcodec_valle_models(device: str = 'cuda'):
    """Load DualCodec-Valle models"""
    print(f"Loading DualCodec-Valle models on device: {device}")
    try:
        ...
        dualcodec_inference_obj = dualcodec.Inference(
            dualcodec_model=dualcodec_model, device=device, autocast=False, w2v_path='/data1/lijiaqi/codebase/w2v-bert-2.0',
        )
        ...
        print(f"Error loading DualCodec-Valle models: {e}")
        raise
...
def load_model_from_config(config_path: str, checkpoint_path: Optional[str] = None, device: str = 'cuda'):
    """Load DualCodec-Valle models (replacing D2Codec model loading)"""
    print(f"Loading DualCodec-Valle models instead of D2Codec model")
    # For DualCodec-Valle, we don't need config files, models are loaded directly
    # This function is kept for compatibility but redirects to DualCodec-Valle loading
    return load_dualcodec_valle_models(device)

def load_voicebox_model_from_config(config_path: str, checkpoint_path: str, device: str = 'cuda'):
    """Voicebox model loading (disabled for DualCodec-Valle)"""
    print(f"Voicebox model loading disabled for DualCodec-Valle inference")
    return None

def load_vocoder(device='cuda'):
    """Load vocoder (disabled for DualCodec-Valle)"""
    print("Vocoder loading disabled for DualCodec-Valle inference")
    return None, None

def prepare_text_tokens(prompt_text: str, target_text: str, language: str) -> Tuple[str, str]:
    """Prepare text for DualCodec-Valle inference (simplified)."""
    # DualCodec-Valle uses preprocess_ref_audio_text for preprocessing
    combined_text = target_text
    print(f"Preparing text for DualCodec-Valle: {combined_text}")
...
def extract_reference_features(
    ...
    """Load reference audio for DualCodec-Valle (simplified)"""
...
def infer_tts_single(
    args,
    models_dict,  # DualCodec-Valle models dictionary
    text: str,
    language: str,
    ref_audio_path: str,
    ref_text: str,
    ...
    decoder_type: str = 'dualcodec_valle',
    duration_top_k: int = 1,
    duration_temperature: float = 1.0
) -> Tuple[torch.Tensor, int, float, float]:
    """Perform TTS inference using DualCodec-Valle"""
    print(f"Performing DualCodec-Valle inference for text: {text}")
    
    print(f"DualCodec-Valle input text: {text}")
    print(f"DualCodec-Valle reference text: {ref_text}")
    print(f"DualCodec-Valle reference audio: {ref_audio_path}")
    
    ar_start_time = time.time()
    
    # Generate audio using DualCodec-Valle inference
    try:
        # Preprocess reference audio and text
        processed_ref_audio, processed_ref_text = preprocess_ref_audio_text(ref_audio_path, ref_text)
        
        # Use DualCodec-Valle inference process
        audio_segment, final_sample_rate, spectrogram = infer_process(
            ar_model_obj=models_dict['ar_model'],
            nar_model_obj=models_dict['nar_model'],
            dualcodec_inference_obj=models_dict['dualcodec_inference_obj'],
            tokenizer_obj=models_dict['tokenizer_model'],
        ...
        if isinstance(audio_segment, np.ndarray):
            generated_audio = torch.from_numpy(audio_segment).float()
        else:
            generated_audio = audio_segment
        ...
        avg_framerate = 0.0  # Not applicable for DualCodec-Valle
        
        print(f"DualCodec-Valle generated audio: {generated_audio.shape}, sample_rate: {sample_rate}")
        
        return generated_audio.cpu().squeeze(), sample_rate, avg_framerate, ar_inference_time
        
    except Exception as e:
        print(f"Error in DualCodec-Valle inference: {e}")
        # Return a dummy audio tensor in case of error
        dummy_audio = torch.zeros(24000)  # 1 second of silence at 24kHz
        return dummy_audio, 24000, 0.0, time.time() - ar_start_time
...
def worker_process(
    ...
    try:
        # Load DualCodec-Valle models
        print(f"Worker {gpu_id}: Loading DualCodec-Valle models")
        models_dict = load_dualcodec_valle_models(device)
        
        # DualCodec-Valle doesn't need second stage models
        voicebox_model = None
        vocoder_decode_func = None
        ...
        for i, test_case in enumerate(test_cases):
            try:
                ...
                # Apply G2P if enabled (disabled for Spark-TTS)
                # if USE_G2P:
                #     if PARTIAL_G2P:
                ...
                decoder_type = 'dualcodec_valle'  # Use DualCodec-Valle
                ...
                # Removed G2P text info for Spark-TTS
                # if USE_G2P:
...
def main():
    parser = argparse.ArgumentParser(description="LibriSpeech Evaluation using DualCodec-Valle")
    
    # DualCodec-Valle Model configuration (models are loaded automatically)
    
    # Model arguments (DualCodec-Valle models are loaded automatically)
    parser.add_argument("--dualcodec_valle_models", type=str, default="automatic", help="DualCodec-Valle models are loaded automatically")
    # Keep these for compatibility but they won't be used
    parser.add_argument("--config", type=str, default="", help="Not used for DualCodec-Valle")
    parser.add_argument("--checkpoint", type=str, default="", help="Not used for DualCodec-Valle")
    parser.add_argument("--voicebox_config", type=str, default="", help="Not used for DualCodec-Valle")
    parser.add_argument("--voicebox_checkpoint", type=str, default="", help="Not used for DualCodec-Valle")
    parser.add_argument("--disable_second_stage", action="store_true", default=True, help="Always true for DualCodec-Valle")
    ...
    # Output arguments
    parser.add_argument('--inference_framerate', type=float, default=1.0, help='Not used for DualCodec-Valle')
    parser.add_argument('--output_dir_base', type=str, default='/data1/lijiaqi/codebase/lab/audio_outputs/', help='output base directory')
    parser.add_argument('--output_dir', type=str, default='dualcodec_valle_librispeech_eval', help='output directory')
    ...
    # Inference parameters (adjusted for DualCodec-Valle)
    parser.add_argument("--beam_size", type=int, default=1, help="Beam size for decoding")
    parser.add_argument("--top_k", type=int, default=15, help="Top-k sampling parameter (DualCodec-Valle default)")
    ...
    print("=" * 50)
    print("DualCodec-Valle LibriSpeech Evaluation")
    ...
    print(f"\nCompleted LibriSpeech DualCodec-Valle TTS inference:")
    ...
    print("🎉 LibriSpeech DualCodec-Valle TTS evaluation completed!")
