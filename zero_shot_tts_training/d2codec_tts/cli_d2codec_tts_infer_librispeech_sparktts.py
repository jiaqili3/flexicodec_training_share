...
def load_sparktts_model(model_path: str, device: str = 'cuda'):
    """Load Spark-TTS model"""
    print(f"Loading Spark-TTS model from: {model_path}")
    try:
        ...
        sparktts = SparkTTS(model_path, device_obj)
        print("Spark-TTS model loaded successfully")
        return sparktts
    except Exception as e:
        print(f"Error loading Spark-TTS model: {e}")
        raise
...
def load_model_from_config(config_path: str, checkpoint_path: Optional[str] = None, device: str = 'cuda'):
    """Load Spark-TTS model (replacing D2Codec model loading)"""
    print(f"Loading Spark-TTS model instead of D2Codec model")
    # For Spark-TTS, we don't need config files, just the model path
    # This function is kept for compatibility but redirects to Spark-TTS loading
    return load_sparktts_model(checkpoint_path if checkpoint_path else config_path, device)

def load_voicebox_model_from_config(config_path: str, checkpoint_path: str, device: str = 'cuda'):
    """Voicebox model loading (disabled for Spark-TTS)"""
    print(f"Voicebox model loading disabled for Spark-TTS inference")
    return None

def load_vocoder(device='cuda'):
    """Load vocoder (disabled for Spark-TTS)"""
    print("Vocoder loading disabled for Spark-TTS inference")
    return None, None

def prepare_text_tokens(prompt_text: str, target_text: str, language: str) -> Tuple[str, str]:
    """Prepare text for Spark-TTS inference (simplified)."""
    combined_text = target_text  # Spark-TTS handles text directly
    print(f"Preparing text for Spark-TTS: {combined_text}")
...
def extract_reference_features(
    ...
    """Load reference audio for Spark-TTS (simplified)"""
...
def infer_tts_single(
    ...
    decoder_type: str = 'sparktts',
    duration_top_k: int = 1,
    duration_temperature: float = 1.0
) -> Tuple[torch.Tensor, int, float, float]:
    """Perform TTS inference using Spark-TTS"""
    print(f"Performing Spark-TTS inference for text: {text}")
    
    print(f"Spark-TTS input text: {text}")
    print(f"Spark-TTS reference text: {ref_text}")
    print(f"Spark-TTS reference audio: {ref_audio_path}")
    
    ar_start_time = time.time()
    
    # Generate audio using Spark-TTS inference
    try:
        # Use Spark-TTS inference method
        wav = model.inference(
            text,
            ref_audio_path,
            prompt_text=ref_text,
            gender=None,  # Can be set based on LibriSpeech metadata if available
            pitch=None,   # Using default
            speed=None    # Using default
        )
        
        # Convert to tensor if it's a numpy array
        if isinstance(wav, np.ndarray):
            generated_audio = torch.from_numpy(wav).float()
        else:
            generated_audio = wav
        ...
        avg_framerate = 0.0  # Not applicable for Spark-TTS
        
        print(f"Spark-TTS generated audio: {generated_audio.shape}, sample_rate: {sample_rate}")
        
        return generated_audio.cpu().squeeze(), sample_rate, avg_framerate, ar_inference_time
        
    except Exception as e:
        print(f"Error in Spark-TTS inference: {e}")
...
def worker_process(
    ...
    try:
        # Load Spark-TTS model
        print(f"Worker {gpu_id}: Loading Spark-TTS model from {args.sparktts_model_path}")
        model = load_sparktts_model(args.sparktts_model_path, device)
        
        # Spark-TTS doesn't need second stage models
        voicebox_model = None
        vocoder_decode_func = None
        ...
        for i, test_case in enumerate(test_cases):
            try:
                ...
                decoder_type = 'sparktts'  # Use Spark-TTS
...
def main():
    parser = argparse.ArgumentParser(description="LibriSpeech Evaluation using Spark-TTS")
    
    # Spark-TTS Model configuration (replace all D2Codec paths)
    sparktts_model_path = 'pretrained_models/Spark-TTS-0.5B'  # Default Spark-TTS model path
    
    # Model arguments (simplified for Spark-TTS)
    parser.add_argument("--sparktts_model_path", type=str, default="/data1/lijiaqi/codebase/Spark-TTS/pretrained_models/Spark-TTS-0.5B", help="Path to Spark-TTS model directory")
    # Keep these for compatibility but they won't be used
    parser.add_argument("--config", type=str, default="", help="Not used for Spark-TTS")
    parser.add_argument("--checkpoint", type=str, default="", help="Not used for Spark-TTS")
    parser.add_argument("--voicebox_config", type=str, default="", help="Not used for Spark-TTS")
    parser.add_argument("--voicebox_checkpoint", type=str, default="", help="Not used for Spark-TTS")
    parser.add_argument("--disable_second_stage", action="store_true", default=True, help="Always true for Spark-TTS")
    ...
    # Output arguments
    parser.add_argument('--inference_framerate', type=float, default=1.0, help='Not used for Spark-TTS')
    parser.add_argument('--output_dir_base', type=str, default='/data1/lijiaqi/codebase/lab/audio_outputs/', help='output base directory')
    parser.add_argument('--output_dir', type=str, default='sparktts_librispeech_eval', help='output directory')
    ...
    print("=" * 50)
    print("Spark-TTS LibriSpeech Evaluation")
    ...
    print(f"\nCompleted LibriSpeech Spark-TTS TTS inference:")
    ...
    print("🎉 LibriSpeech Spark-TTS TTS evaluation completed!")
