...
def load_firered_models(config_path: str, pretrained_path: str, device: str = 'cuda'):
    """Load FireRed TTS models"""
    print(f"Loading FireRed TTS model on device: {device}")
    try:
        tts = FireRedTTS(
            config_path=config_path,
            pretrained_path=pretrained_path,
        )
        print("FireRed TTS model loaded.")
        
        return {
            'tts_model': tts
        }
    except Exception as e:
        print(f"Error loading FireRed TTS model: {e}")
        raise
...
def load_model_from_config(config_path: str, checkpoint_path: Optional[str] = None, device: str = 'cuda'):
    """Load FireRed TTS models (replacing D2Codec model loading)"""
    print(f"Loading FireRed TTS model instead of D2Codec model")
    # For FireRed TTS, config_path is the JSON config and checkpoint_path is the pretrained_path
    return load_firered_models(config_path, checkpoint_path, device)

def load_voicebox_model_from_config(config_path: str, checkpoint_path: str, device: str = 'cuda'):
    """Voicebox model loading (disabled for FireRed TTS)"""
    print(f"Voicebox model loading disabled for FireRed TTS inference")
    return None

def load_vocoder(device='cuda'):
    """Load vocoder (disabled for FireRed TTS)"""
    print("Vocoder loading disabled for FireRed TTS inference")
    return None, None

def prepare_text_tokens(prompt_text: str, target_text: str, language: str) -> Tuple[str, str]:
    """Prepare text for FireRed TTS inference (simplified)."""
    # FireRed TTS uses simple text input
    combined_text = target_text
    print(f"Preparing text for FireRed TTS: {combined_text}")
...
def extract_reference_features(
    ...
    """Load reference audio for FireRed TTS (simplified)"""
...
def infer_tts_single(
    args,
    models_dict,  # FireRed TTS models dictionary
    text: str,
    language: str,
    ref_audio_path: str,
    ref_text: str,
    ...
    decoder_type: str = 'firered_tts',
    duration_top_k: int = 1,
    duration_temperature: float = 1.0
) -> Tuple[torch.Tensor, int, float, float]:
    """Perform TTS inference using FireRed TTS"""
    print(f"Performing FireRed TTS inference for text: {text}")
    
    print(f"FireRed TTS input text: {text}")
    print(f"FireRed TTS reference text: {ref_text}")
    print(f"FireRed TTS reference audio: {ref_audio_path}")
    
    ar_start_time = time.time()
    
    # Generate audio using FireRed TTS inference
    try:
        tts_model = models_dict['tts_model']
        
        # Use FireRed TTS synthesize method
        rec_wavs = tts_model.synthesize(
        ...
        generated_audio = rec_wavs.detach().cpu()
        
        # FireRed TTS outputs at 24kHz
        sample_rate = 24000
        ...
        avg_framerate = 0.0  # Not applicable for FireRed TTS
        
        print(f"FireRed TTS generated audio: {generated_audio.shape}, sample_rate: {sample_rate}")
        
        return generated_audio.squeeze(), sample_rate, avg_framerate, ar_inference_time
        
    except Exception as e:
        print(f"Error in FireRed TTS inference: {e}")
...
def worker_process(
    ...
    try:
        # Load FireRed TTS models
        print(f"Worker {gpu_id}: Loading FireRed TTS model")
        models_dict = load_firered_models(
            config_path=args.config, 
            pretrained_path=args.checkpoint, 
            device=device
        )
        
        # FireRed TTS doesn't need second stage models
        voicebox_model = None
        vocoder_decode_func = None
        ...
        for i, test_case in enumerate(test_cases):
            try:
                ...
                # Apply G2P if enabled (disabled for FireRed TTS)
                # if USE_G2P:
                #     if PARTIAL_G2P:
                ...
                # Determine language for FireRed TTS
                # For LibriSpeech (English), use "en", for Chinese use "zh"
                lang_code = 'en'  # LibriSpeech is English
                
                decoder_type = 'firered_tts'  # Use FireRed TTS
                audio, sample_rate, avg_framerate, ar_inference_time = infer_tts_single(
                    args=args,
                    models_dict=models_dict,
                    text=text_final.lower(),  #.replace('.', ','),
                    language=lang_code,  # Use language code for FireRed TTS
                    ref_audio_path=ref_audio_path,
...
def main():
    parser = argparse.ArgumentParser(description="LibriSpeech Evaluation using FireRed TTS")
    
    # FireRed TTS Model configuration
    parser.add_argument("--config", type=str, default='/data1/lijiaqi/codebase/FireRedTTS/FireRedTTS/configs/config_24k.json', help="Path to FireRed TTS config JSON file (e.g., configs/config_24k.json)")
    parser.add_argument("--checkpoint", type=str, default='/data1/lijiaqi/codebase/FireRedTTS', help="Path to FireRed TTS pretrained models directory")
    
    # Keep these for compatibility but they won't be used
    parser.add_argument("--voicebox_config", type=str, default="", help="Not used for FireRed TTS")
    parser.add_argument("--voicebox_checkpoint", type=str, default="", help="Not used for FireRed TTS")
    parser.add_argument("--disable_second_stage", action="store_true", default=True, help="Always true for FireRed TTS")
    ...
    # Output arguments
    parser.add_argument('--inference_framerate', type=float, default=1.0, help='Not used for FireRed TTS')
    parser.add_argument('--output_dir_base', type=str, default='/data1/lijiaqi/codebase/lab/audio_outputs/', help='output base directory')
    parser.add_argument('--output_dir', type=str, default='firered_tts_librispeech_eval', help='output directory')
    ...
    # Inference parameters (adjusted for FireRed TTS)
    ...
    print("=" * 50)
    print("FireRed TTS LibriSpeech Evaluation")
    ...
    print(f"\nCompleted LibriSpeech FireRed TTS inference:")
    ...
    print("🎉 LibriSpeech FireRed TTS evaluation completed!")
